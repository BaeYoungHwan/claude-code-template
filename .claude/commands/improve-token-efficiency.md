...
**스텝 0 — 캐시 확인 (새 세션 없으면 재분석 생략)**

다음 조건을 만족하면 캐시된 대시보드를 반환하고 재분석을 생략한다:
- `.cache/token-efficiency-last-run` 파일이 존재하고
- 해당 타임스탬프 이후 `~/.claude/projects/<repo>/` 에 새 `.jsonl` 파일이 없는 경우

조건 불만족 시 → 정상 분석 진행 후 `.cache/token-efficiency-last-run` 타임스탬프 갱신
...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/improve-token-efficiency/scripts/analyze_sessions.py" \
    --repo "$(pwd)" \
...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/improve-token-efficiency/scripts/build_dashboard.py" \
