...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/ai-readiness-cartography/scripts/score.py" <repo-path> \
...
- `.claude/skills/ai-readiness-cartography/assets/template.html` — 복사 후 채울 원본 대시보드
- `.claude/skills/ai-readiness-cartography/references/scoring-rubric.md` — 7 카테고리 채점 기준 v2
- `.claude/skills/ai-readiness-cartography/scripts/score.py` — 자동 채점 + ROI 액션 생성 (Python 3.10+, stdlib only)
